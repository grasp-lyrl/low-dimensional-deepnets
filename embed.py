...
def main():
    ...
    groupby = ['m', 'opt', 't']
    d = load_d(loc, cond={'bs': [200], 'aug': [True],
               'wd': [0.0], 'bn': [True]}, avg_err=True)
    d = drop_untrained(d, key='err', th=0.01, verbose=False).reset_index()
    d = avg_model(d, groupby=groupby, probs=False, get_err=True, update_d=True,
    ...
    configs = product(d['m'].unique(), d['opt'].unique(), d['seed'].unique())
