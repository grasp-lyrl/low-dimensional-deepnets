...
def avg_by_new_t(k, loc='results/models/reindexed', groups=['aug', 'm', 'opt', 'bs', 'lr', 'wd', 't']):
    ...
    d_avg = avg_model(d, probs=True, groupby=groups+['t'], update_d=False)
...
def main():
    ...
    with mp.Pool(processes=4) as pool:
        pool.map(
            partial(avg_by_new_t, loc=loc, groups=groups), indices.keys(), chunksize=1)
